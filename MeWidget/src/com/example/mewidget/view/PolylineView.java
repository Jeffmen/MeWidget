...
public class PolylineView extends View {
	...
	private List<ArrayList<PointF>> pointFs;// 数据列表
	...
	private void initData() {
		pointFs = new ArrayList<ArrayList<PointF>>();
		ArrayList<PointF> data = new ArrayList<PointF>(); 
        data.add(new PointF(0.3F, 0.5F));  
        data.add(new PointF(1F, 2.7F));  
        data.add(new PointF(2F, 3.5F));  
        data.add(new PointF(3F, 3.2F));  
        data.add(new PointF(4F, 1.8F));  
        data.add(new PointF(5F, 1.5F));  
        data.add(new PointF(6F, 2.2F));  
        data.add(new PointF(7F, 5.5F));  
        data.add(new PointF(8F, 7F));  
        data.add(new PointF(8.6F, 5.7F)); 
        pointFs.add(data);
	}
	...
	private void drawPolyline(Canvas canvas) {
		for(int j = 0; j < pointFs.size(); j++){
			 ...
			 * 生成Path和绘制Point
			 */
			for (int i = 0; i < pointFs.get(j).size(); i++) {
				// 计算x坐标
				float x = mCanvas.getWidth() / maxX * pointFs.get(j).get(i).x + left;
	
				// 计算y坐标
				float y = mCanvas.getHeight() / maxY * pointFs.get(j).get(i).y;
				y = mCanvas.getHeight() - y + top + spaceY;
	
				// 绘制小点点
				canvas.drawCircle(x, y, thickLineWidth, pointPaint);
	...
	private void drawLines(Canvas canvas) {
		...
		for(int i=0; i < pointFs.size(); i++){
			tempCount = pointFs.get(i).size();
			count =  tempCount > count ? tempCount : count;
			for (int j = 0; j < tempCount; j++) {
				if (maxX < pointFs.get(i).get(j).x) {
					maxX = pointFs.get(i).get(j).x;
				}
			}
		}
		...
		maxY = 0;
		for(int i=0; i < pointFs.size(); i++){
			for (int j = 0; j < pointFs.get(i).size(); j++) {
				if (maxY < pointFs.get(i).get(j).y) {
					maxY = pointFs.get(i).get(j).y;
				}
			}
		}
		// 计算纵轴最近的能被count整除的值
	 ...
	 *            点集合
	 */
	public synchronized void setData(List<ArrayList<PointF>> pointFs, String signX, String signY) {
		/*
		 * 数据为空直接GG
		 */
